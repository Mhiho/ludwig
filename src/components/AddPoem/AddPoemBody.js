...
class AddPoemBody extends Component {
  ...
  componentDidMount() {
    ...
    this.setState({ id: id }, () => {
      ...
      fetch(url, {
        ...
        .then((data) => {
          this.setState(
            { title: data[0].title, poem: data[0].poem, isLoading: false },
            () => {
  ...
  oneDown(id) {
    ...
    this.setState(
      { poem: update(this.state.poem, { akapits: { $set: newArr } }) },
      () => {
        fetch(url, {
          method: "PATCH",
          headers: headers,
        ...
        poem: update(this.state.poem, {
          akapits: {
            [this.state.poem.akapits.length - 1]: {
              lines: {
                [this.state.poem.akapits[this.state.poem.akapits.length - 1]
                  .lines.length]: { $set: this.state.line },
              },
            },
          },
        }),
  ...
  render() {
    ...
    return (
      <div className={classes2.Frame}>
        ...
        <span>Dodaj akapit </span>
        <button className={classes2.addAkaBtn} onClick={() => this.addAka()}>
          +
        </button>
        ...
        <div className={classes.Part}>
          {this.state.poem.akapits.map((akapit, indAka) => (
            <div key={indAka} className={classes2.Akapit}>
              {" "}
              {akapit.lines.map((line, index) => (
                <p className={classes2.Line} key={index + 100}>
                  {line}{" "}
                </p>
              ))}{" "}
              <span
              ...
              <span
                ...
                onClick={
                  this.state.poem.akapits.length - 1 === indAka
                    ? null
                    : () => this.oneDown(indAka)
                }
              >
        ...
        <button
          className={classes2.goBackBtn}
          onClick={() => this.props.history.goBack()}
        >
          Wróć do poprzedniej strony
        </button>
        <button
          className={classes2.deleteBtn}
          onClick={() => this.deletePoem(this.state.id)}
        >
          Usuń ten bullshit
        </button>
      </div>
